...
#    - Remove duplicadas
#
# 3) Timestamp da última execução em BD_Config!C2
#    da planilha 1-_lTKT4wSDlJtTXkF1tLHstV9h-S3Yq_2cE8jOIC3kI
#
# Regras comuns:
...
# --- Etapa 3: Timestamp da última execução ---
...
def gravar_timestamp(sheets_svc):
    """Grava o timestamp atual em BD_Config!C2 da planilha de destino."""
    agora = datetime.now(ZoneInfo(TIMESTAMP_TIMEZONE))
    timestamp_str = agora.strftime("%d/%m/%Y %H:%M:%S")
    range_destino = f"{TIMESTAMP_SHEET_NAME}!{TIMESTAMP_CELL}"
    ...
    try:
        ...
        print(f"🕒 Timestamp gravado em {TIMESTAMP_SHEET_NAME}!{TIMESTAMP_CELL}: {timestamp_str}")
    except HttpError as e:
        print(f"❌ Erro ao gravar timestamp em {TIMESTAMP_SHEET_NAME}!{TIMESTAMP_CELL}: {e}")
